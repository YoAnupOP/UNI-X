...
const getInboxSnapshotCached = unstable_cache(
    async (viewerId: string): Promise<Conversation[]> => {
    ...
    ['dm-inbox-snapshot'],
    { revalidate: 15, tags: [CACHE_TAGS.dmInbox] }
)
...
export async function getInboxSnapshot(): Promise<Conversation[]> {
    ...
    return getInboxSnapshotCached(viewer.user.id)
}
